...
function documento(data,{title,description,url,image,body,sign}){
  ...
  return `<!doctype html>
<html lang="es-MX" data-studio-theme="light">
<head>
<meta charset="utf-8">
...
function selector(data,actual){return `<nav class="hs-otras" aria-label="Horóscopo de los doce signos"><p class="ceja">También puedes leer otro signo</p><div class="hs-otras-lista">${data.signs.map(s=>`<a href="/horoscopo-semanal/${s.slug}"${s.slug===actual?' aria-current="page"':''}>${esc(s.name)}</a>`).join('')}</div></nav>`;}
...
function renderIndex(data){
  ...
  const body=`<section class="hs-hero" aria-labelledby="titulo"><div><p class="ceja">Predicciones · Los doce signos</p><h1 id="titulo">Tu signo.<em>Una semana por explorar.</em></h1><p class="hs-fechas">Del <time datetime="${data.start}">${esc(rangoTexto)}</time></p><p class="hs-entrada">${esc(data.intro)}</p><div class="hs-edicion">${estado(data)}<span>Publicado el ${esc(formatDate(data.publishedAt.slice(0,10)))}</span></div><div class="hf-acciones"><a class="hf-boton principal" href="#signos">Elegir mi signo ↓</a><a class="hf-boton" href="#cielo">Ver los aspectos de la semana</a></div></div>${portada(portadaSigno.slug,portadaSigno.name,'Doce miradas al mismo cielo')}</section>
<section class="hs-cielo" aria-label="Momentos destacados"><p class="ceja">Tres claves<br>de la semana</p><div class="hs-cielo-lista">${data.events.slice(0,3).map(e=>`<a href="#evento-${esc(e.id)}"><time datetime="${esc(e.date)}">${esc(fechaCorta(e.date))}</time>${esc(e.label)} <span aria-hidden="true">↗</span></a>`).join('')}</div></section>
<section class="hs-seccion" id="signos"><div class="hs-seccion-cabecera"><div><p class="ceja">Amor · Trabajo · Dinero</p><h2>Elige tu signo zodiacal</h2></div><p>Empieza por tu signo solar. Encontrarás una lectura general y los movimientos planetarios que le dan contexto.</p></div><div class="hs-signos">${data.signs.map(s=>`<a class="hs-signo" data-elemento="${esc(s.element.toLowerCase())}" href="/horoscopo-semanal/${s.slug}"><div class="hs-signo-arte"><img src="/assets/zodiaco/${s.slug}-celestial.webp" alt="" width="600" height="450" loading="lazy" decoding="async"></div><p class="hs-elemento">${esc(s.element)}</p><h3>${esc(s.name)} <span aria-hidden="true">${esc(s.symbol)}</span></h3><p>${esc(s.headline)}</p></a>`).join('')}</div></section>
${eventos(data)}${metodo(data)}
<section class="hf-cierre"><div><p class="ceja">Tu cielo personal</p><h2>Hay mucho más que tu signo solar.</h2><p>Tu carta natal y tus tránsitos consideran el momento y lugar de nacimiento.</p></div><a class="hf-boton" href="/astroplanetario.html?view=natal&amp;tab=transitos">Explorar mis tránsitos →</a></section>`;
