...
function analizar(carta,{profesional=root.NatalProfesional,regencias=root.NatalRegencias,estructura=root.NatalEstructura,patrones=root.NatalPatrones,relaciones=root.NatalRelaciones,luminarias=root.NatalLuminarias,estrellas=root.NatalEstrellas,puntos=root.NatalPuntos,D=root.Dignidades,E=root.Efem}={}){
  if(!carta||!Array.isArray(carta.cuerpos))throw new TypeError("Falta una carta natal calculada.");
  if(!profesional||!regencias||!estructura||!patrones||!relaciones||!luminarias||!estrellas||!puntos)throw new TypeError("Faltan módulos profesionales para construir la síntesis.");
  const condicion=profesional.analizar(carta,E),gobierno=regencias.analizar(carta,{D,E}),forma=estructura.analizar(carta),geometria=patrones.analizar(carta),avanzadas=relaciones.analizar(carta,E),luz=luminarias.analizar(carta,E,regencias),firmamento=estrellas.analizar(carta,E),sensibles=puntos.analizar(carta);
  ...
  const puntosTexto=`Se revisan ${sensibles.resumen.puntos} puntos sensibles y ${sensibles.resumen.ejes} ejes disponibles. Hay ${sensibles.resumen.contactos} contactos geométricos dentro de los orbes declarados, ${sensibles.resumen.partiles} de ellos partiles. ${sensibles.conHora?'Los dos ejes angulares y la Parte de la Fortuna se calculan con la hora y el lugar natales.':'Sin hora se conservan únicamente el eje nodal y Lilith media; no se estiman ángulos ni Fortuna.'}`;
  ...
  pasos.push("Revisar los ejes y puntos sensibles como referencias geométricas, sin convertir sus contactos en dignidad planetaria.");
  ...
  return{horaConocida:!!carta.datos?.horaConocida,condicion,gobierno,estructura:forma,geometria,avanzadas,luminarias:luz,estrellas:firmamento,puntos:sensibles,prioridades,masSostenidos,masExigidos,fortalezas,revisar,pasos,lecturas:[
    ...
    {id:"puntos",titulo:"Ejes y puntos sensibles",texto:puntosTexto},
