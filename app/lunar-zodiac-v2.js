...
function injectStyles(){
  ...
  const s=document.createElement('style');s.id='lunar-zodiac-v2-style';s.textContent=`
    ...
    .zv2-art{position:relative;width:100px;height:100px;display:grid;place-items:center;border-radius:16px;border:1px solid rgba(218,187,111,.28);background-color:#060b18;background-image:url('assets/zodiac/zodiac-sprite.webp?v=3');background-size:400% 300%;background-repeat:no-repeat;box-shadow:inset 0 0 28px rgba(218,187,111,.05),0 0 18px rgba(218,187,111,.06);transition:background-position .18s ease,filter .18s ease;overflow:hidden}
...
function render(){
  ...
  const idx=SIGN_INDEX[name],col=idx%4,row=Math.floor(idx/4),x=col/3*100,y=row/2*100;
  const art=document.getElementById('heroZodiacArt');
  if(art){art.style.backgroundPosition=`${x}% ${y}%`;art.dataset.sign=name;art.setAttribute('aria-label',`Ilustración zodiacal premium de ${name}`)}
