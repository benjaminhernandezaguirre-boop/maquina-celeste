(()=>{'use strict';
const IMAGE_SLUG={'Aries':'aries','Tauro':'tauro','Géminis':'geminis','Cáncer':'cancer','Leo':'leo','Virgo':'virgo','Libra':'libra','Escorpio':'escorpio','Sagitario':'sagitario','Capricornio':'capricornio','Acuario':'acuario','Piscis':'piscis'};
...
function injectStyles(){
  ...
  const s=document.createElement('style');s.id='lunar-zodiac-v2-style';s.textContent=`
    ...
    .zv2-art-img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;pointer-events:none;user-select:none}
    ...
    @media(min-width:601px){.hero-copy{padding-top:7rem}}
...
function build(){
  ...
  const img=document.createElement('img');img.className='zv2-art-img';img.id='heroZodiacImage';img.alt='';img.setAttribute('aria-hidden','true');art.appendChild(img);
...
function render(){
  ...
  const art=document.getElementById('heroZodiacArt'),img=document.getElementById('heroZodiacImage');
  if(img){const src=`assets/zodiaco/${IMAGE_SLUG[name]}-celestial.webp`;if(!img.src.endsWith(src))img.src=src}
