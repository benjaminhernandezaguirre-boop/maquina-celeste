...
test('empty hour, duplicate city and invalid date invalidate old map with an explanation',()=>{
  ...
  a.fill('lugar','Madrid, España');a.fill('fecha','1700-01-01');a.draw();assert.ok(a.node('aviso').textContent.includes('1800'));
});
...
test('saving valid natal records preserves incompatible legacy records',()=>{
  ...
  vm.runInContext(fs.readFileSync(path.join(root,'app/ciudades.js'),'utf8'),c);
